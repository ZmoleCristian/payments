...
#[test]
fn wcgw2_rejects_five_decimal_places() {
    let o = drive("type,client,tx,amount\ndeposit,1,1,1.00005\n").expect("run");
    assert!(o.stderr.contains("line 2: invalid amount"), "stderr: {}", o.stderr);
    assert!(!o.stdout.contains('1'), "rejected deposit must not appear: {}", o.stdout);
}
...
#[test]
fn wcgw5_zero_amount_records_disputable_tx() {
    let o = drive("type,client,tx,amount\ndeposit,1,1,0\ndispute,1,1,\nresolve,1,1,\n").expect("run");
    assert_eq!(row_for(&o.stdout, 1), "1,0.0000,0.0000,0.0000,false");
    assert!(o.stderr.is_empty(), "zero dispute cycle must be clean: {}", o.stderr);
}
