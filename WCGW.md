...
34. [AMENDED by 57] Broken stdout pipe mid-render — write error becomes FatalError, nonzero exit; no silent truncation. [d]
...
54. REVERSES 51. There is no "overflow" in the total column — a numeric column emits numbers or the run is wrong. Printing a word there hands every consumer an unparseable field while exit 0 claims a clean run, which is #2/#3 relocated into the reader. total = available + held is an ACCOUNT INVARIANT, so a row that would make it unrepresentable is a bad row: reject it at apply time (BadAmount) like every other overflow (#6) and the account never reaches render unrepresentable. Render still checks, and treats a breach as FatalError::Corrupt — unreachable by construction, which is the point. [d]
55. A rejected row moves NO money. Ops touching both available and held computed the first, assigned it, then let the second fail through `?` — leaving the account half-mutated while the row was reported rejected. Compute every new value AND the total invariant first, assign only once all of them succeed. A "reject" that silently ate a deposit is worse than a crash: the report says the row did nothing and the ledger disagrees. [d]
56. AMENDS 52. Leading and trailing SPACES around a field are partner padding, not content — the spec's own sample is `deposit, 1, 1, 1.0`. Spaces before a quote do not stop it from opening the field (`, "3"` is a quoted 3) and spaces after a closing quote are skipped (`"4" ,`). What stays malformed is a quote in the MIDDLE of unquoted content (`depo"sit`): that is a real grammar break, not whitespace sloppiness. [d]
57. AMENDS 34. A reader closing the pipe is not our failure. `payments x.csv | head -1` is a normal thing to do, and every well-behaved unix tool answers EPIPE on its OUTPUT by stopping quietly with exit 0 — that is why `yes | head` does not scream. #34 conflated two events that share an error type: a device that failed mid-write (ENOSPC, exit 1, loud) and a consumer that left on purpose (exit 0, silent). Only the first is ours. The distinction is made in main.rs, which owns the process and knows what stdout is; the library still returns the io error and decides nothing. The report sink is untouched by this — EPIPE there is still FatalError::Report (#36), because a diagnostic nobody can see is a bug. [d]
58. The row path allocates NOTHING per row. One reused Record (a byte buffer plus field bounds) and one reused Scan carry every record, so parsing and reporting 9000 rows costs exactly what 1000 costs. The old shape allocated a Vec<Vec<u8>> per record and a String per row from `to_ascii_lowercase()` on the type field — twice, once in parse_row and once in no_amount_kind — and at 42's thousands of concurrent streams a per-row allocation is the whole budget. Case-insensitive matching is `eq_ignore_ascii_case`, never a lowercased copy. The only growth left is the transactions map, which 38 requires. Proven in tests/allocs.rs, not asserted in prose. [d]
59. The suite carries an INDEPENDENT model, not a mirror. tests/oracle.rs replays random op sequences against a model written from these rules in i128 and compares every account; self-consistency checks like total == available + held pass happily on a ledger that silently lost every deposit, so they are not an oracle. The id pools are deliberately tiny (clients 0..4, txs 0..8): with random u16 ids a dispute almost never names a real tx, the state machine is never reached, and the suite proves nothing while looking thorough. Amounts reach i64::MAX so the overflow arms are real. [d]
